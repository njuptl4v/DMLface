...
from nets.layers import SSH, SPPF
...
class RetinaFace(nn.Module):
    def __init__(self, cfg=None, pretrained=False, mode='train'):
        ...
        self.conv3 = make_three_conv([64, 128], 128)  #两层特征融合，变128通道，做3此卷积加深变40*40*64
        
        self.upsample2 = Upsample(64, 32)    #fpn中间层上采样，80*80*32
        self.conv_for_P3 = conv_bn1X1(64, 32, stride=1)   #最底层改变通道数，80*80*32
        self.conv4 = make_three_conv([32, 64], 64)   #最底层和中间层融合，变64通道，做3此卷积加深变80*80*32
        
        self.fpn_head1 = conv_bn1X1(32, 64, stride=1)  #输出最底层80*80*64
        
        self.down_sample1 = conv_bn(32, 64, stride=2)   #conv4做步长为2的卷积相当于下采样，40*40*64
        ...
        self.down_sample2 = conv_bn(64, 128, stride=2)    #conv5上采样，20*20*128
    ...
    def forward(self, inputs):
        ...
        p1 = torch.cat([p1, p2_upsample], axis=1)   #40*40*128
        p1 = self.conv3(p1)     #40*40*64
        p1_upsample = self.upsample2(p1)   #80*80*32
        p0 = self.conv_for_P3(x0)   #80*80*32
        p0 = torch.cat([p0, p1_upsample], axis=1)   #80*80*64
        c0 = self.conv4(p0)   #80*80*32,一会输出用
        p0_downsample = self.down_sample1(c0)  #40*40*64
        p3 = torch.cat([p0_downsample, p1], axis=1)  #40*40*128
        c1 = self.conv5(p3)   #40*40*64,一会输出用
        p1_downsample = self.down_sample2(c1)   #20*20*128
        ...
        #   最底层特征80*80*32
        #-------------------------------------------#
        out2 = self.fpn_head3(c2)
        out1 = self.fpn_head2(c1)
        out0 = self.fpn_head1(c0)
        #输出20*20*64  40*40*64  80*80*64

        feature1 = self.ssh1(out0)
        feature2 = self.ssh2(out1)
        feature3 = self.ssh3(out2)
